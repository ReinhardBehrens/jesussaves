...
- CI builds each architecture on a matching Mac runner and tests the bundled app, its signature, 4K output and animation. Intel checks both CPU and GPU renderers; ARM checks the GPU renderer. Both check a live GPU window. CI also installs Apple Rosetta and checks the Intel CPU renderer on an ARM Mac. There is no claim of physical-monitor or System Settings integration testing.
