...
The per-user PowerShell installer copies the runtime and writes the selected `.scr` path to `HKCU\Control Panel\Desktop\SCRNSAVE.EXE`. It calls `SystemParametersInfoW` with `SPI_SETSCREENSAVETIMEOUT` (300 seconds by default) and `SPI_SETSCREENSAVEACTIVE`, using profile-update and change-notification flags. It then reads back both live values and the selected path. This avoids relying on an installation dialog to apply a selection. Failed setting updates restore the previous state; the initial state is also kept in a JSON backup. Sign-in protection is never modified. Centrally managed selection/timeout policies are reported instead of overwritten.

The installer test starts with no selected saver and a zero timeout, checks the installed files, validates both registry and live API state, tests a custom wait and reinstall, and opens Screen Saver Settings to read the actual selected name and displayed wait. It verifies preview shutdown when the panel closes. Windows preview event handling responds to SDL close events and the disappearance or hiding of the host after it has become visible.

References: [selected screensaver registry entry](https://learn.microsoft.com/en-us/windows/win32/devnotes/scrnsave-exe) and [SystemParametersInfoW](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-systemparametersinfow).
